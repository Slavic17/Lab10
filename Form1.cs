...
namespace Lab10OOP
{
    public partial class Form1 : Form
    {
        ...
        private DSA dsa; // Для цифрового підпису
        ...
        public Form1()
        {
            ...
            {
                MessageBox.Show($"Error initializing DSA: {ex.Message}");
            }
        }

        // Метод 1: блоковий шифр KHUFU (демонстраційно використовуємо AES)
        ...
        // Метод для розшифрування блокового шифру KHUFU (AES)
        ...
        // Метод 2: хеш-функція HAVAL (демонстраційно використовуємо SHA256)
        public byte[] HavalHash(string input)
        {
        ...
        // Метод 3: цифровий підпис ДЕРЖСТАНДАРТ (DSA)
        public byte[] CreateSignature(string plaintext)
        {
            ...
            {
                MessageBox.Show($"An error occurred during signing: {ex.Message}");
                return null;
            }
        }

        public bool VerifySignature(string plaintext, byte[] signature)
        {
            byte[] data = Encoding.UTF8.GetBytes(plaintext);
            try
            {
            ...
            {
                MessageBox.Show($"An error occurred during signature verification: {ex.Message}");
                return false;
            }
        }
        ...
        private async Task DrawRect(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                byte[] hash = HavalHash(Convert.ToBase64String(encryptedData));
        ...
        // Асинхронний метод для генерації випадкових чисел і створення цифрового підпису
        private async Task GenerateRandomNumbers(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Створення цифрового підпису для випадкового числа
                    byte[] signature = CreateSignature(randomNumber);
                    ...
                    // Конвертація підпису в base64 рядок
                    string signatureString = Convert.ToBase64String(signature);

                    // Оновлення річтекстбоксу підписаним випадковим числом
                    richtextbox1.Invoke((MethodInvoker)delegate ()
                    {
                        richtextbox1.Text += signatureString + Environment.NewLine;
                    });
        ...
        // Обробник події кнопки для генерації випадкових чисел і створення цифрового підпису
        ...
        // Обробник події кнопки для скасування генерації випадкових чисел і створення цифрового підпису
